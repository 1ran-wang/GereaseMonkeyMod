var repeat_regex = ["https:?\/\/.*?hls.*?\.m3u8",
...
function selector_one_by_one(x) {
...
function adblock4limbo(x, csp) {
    ...
    document.querySelector('html').insertBefore(new_body, body);
    ...
    if (csp == 'nocsp') {
        ...
        var nocsp =
            ...
            'background-color:transparent;background-image:url("' + icon_url +
            '") !important;background-size:100% !important;background-repeat:no-repeat;';
        x4Home.style = nocsp;
        document.getElementById('x4Div').appendChild(x4Home);
    ...
    if (document.getElementById('x4Home')) {
        ...
        setTimeout(() => {
            if (last_known_scroll_position !== window.scrollY &&
                document.querySelector('img.lockscreen') == null) {
                if (document.querySelector("#navigation4limbo").style
                    .zIndex > 0) {
                    document.querySelector("#navigation4limbo").style
                        .zIndex = ++document.querySelector(
                            "#navigation4limbo").style.zIndex;
                }
                znsh_unlock();
                x4Home_button('1');
                if (click_dont_move_and_click < 400) {
                    click_dont_move_and_click =
                        click_dont_move_and_click + 600;
                } else {
                    click_dont_move_and_click =
                        click_dont_move_and_click + 120;
                }
                document.querySelector('#x4Home').style.zIndex =
                    document.querySelector('#x4Home').style.zIndex + 1;
            } else {
                x4Home_button('0');
...
function _onclick_button() {
    if (document.querySelector('button#x4Home')) {
        document.querySelector('button#x4Home').addEventListener("click",
            function() {
                body_build('true');
            })
    }
    setTimeout(() => {
        if (document.querySelector("button#xX")) {
            document.querySelector('button#xX').addEventListener(
                "click",
                function() {
                    body_build('false');
                })
        ...
        if (document.querySelector('button#webChat')) {
            document.querySelector('button#webChat').addEventListener(
                "click",
                function() {
                    crisp_active('1');
                })
        ...
        if (document.querySelector('button#hidedaohang')) {
            document.querySelector('button#hidedaohang')
                .addEventListener("click", function() {
                    hide_button_switch('1');
                })
        }
        if (document.querySelector('a#lsj')) {
            document.querySelector('a#lsj').addEventListener("click",
                function() {
                    open_googlesearch_iframe();
                })
        }
        if (document.getElementById('nsfwmode_switch')) {
            document.getElementById('nsfwmode_switch').addEventListener(
                'click',
                function() {
                    nsfwmode();
                })
        ...
        if (document.getElementById('lock_screen')) {
            document.getElementById('lock_screen').addEventListener(
                'click',
                function() {
                    lock_screen_switch();
                })
...
var nsfw_regex = new RegExp(
    /\b(thisav|njav|missav|javlib|attackers|18comic|javday|hamnime|takara|tameikegoro|deeps|moodyz|s1s1s1|nagae|ideapocket|dasdas|oppai|kawaii|satsu|mgstage|manji-group|rocket|muku|dmm|beauty|gloryquest|supjav|jable|xvideos|pornhub|porn|wnacg|av)\b/i
    );
var csp_regex = new RegExp(/\b(twitter|xvideos)\b/i);
var echo_ell_length = 21;
var echo_ell_overlay_length = 1;
...
var number_x = 0;
if (/\b(google|bing)\b/i.test(window.location.href.toLowerCase())) {} else if (
    csp_regex.test(window.location.href.toLowerCase()) && !(/\b(mobile)\b/i
        .test(navigator.userAgent.toLowerCase()))) {
    adblock4limbo(x4Home_button_width(), 'csp');
    _onclick_button();
} else {
    adblock4limbo(x4Home_button_width(), 'nocsp');
    _onclick_button();
}
...
function csp_remove() {
    ...
    if (csp_regex.test(window.location.href.toLowerCase())) {
        if (document.querySelector("button#x4Home") !== null && document
            .querySelector("script[src*='Adblock4limbo.function.js']") !== null
            ) {} else {
...
css_add(file.global_css, 'navigation4limbo_style');

function insertBefore_that_element_xx(x) {
    let parentElement = document.querySelector('div#rightcolumn')
    let insertBefore_that_element_X = document.querySelector(
        'div#video_favorite_edit');
...
function div_sample(echo_selector, insertBefore_that_element, switchX,
    ...
    div_title.className = 'title_global';
    ...
    if (switchX == 1) {
        let insertBefore_that_elementx = document.querySelector(
            insertBefore_that_element);
        target.insertBefore(div_global_x, insertBefore_that_elementx);
    } else {
...
function scroll_switch() {
    ...
    for (i = 0; i < div_global.length; i++) {
        ...
        if (div_global[i].querySelectorAll('li').length * 31 + fbt > 8 * 31 ||
            ...
            document.querySelectorAll("div.div_global")[i].appendChild(
                scroll_innerHTML);
        }
    }
}
...
function xXX() {
    ...
    var a_length = document.querySelectorAll('.div_global')[3].querySelectorAll(
        "li").length;
    for (i = 0; i < a_length; i++) {
        if (document.querySelectorAll('.div_global')[3].querySelectorAll("a")[i]
            .clientHeight > 27) {
            let scroll_innerHTML = document.createElement('div');
            scroll_innerHTML.textContent = '*可向上滑动查看更多';
            scroll_innerHTML.className = "scroll";
...
function body_bgp_switch() {
    if (!(csp_regex.test(window.location.href.toLowerCase()))) {
        let url_w =
            "https://raw.githubusercontent.com/limbopro/Adblock4limbo/main/Adguard/Adblock4limbo_bgp_w.jpg";
        let url_h =
            "https://raw.githubusercontent.com/limbopro/Adblock4limbo/main/Adguard/Adblock4limbo_bgp.jpg";
        if (window.innerWidth * 0.65 >= window.innerHeight) {
            document.querySelector("div#navigation4limbo").style
                .backgroundImage = `url(${url_w})`;
        } else {
            document.querySelector("div#navigation4limbo").style
                .backgroundImage = `url(${url_h})`;
        }
    } else {
        document.querySelector("div#navigation4limbo").style.backgroundColor =
            '#dfdfdf';
        document.querySelector("div > button#xX").textContent = 'X';
        document.querySelector("div > button#xX").style =
            'font-size:-webkit-xxx-large;border-radius:50%;';
    }
}

function echo_align() {
    if (window.innerHeight > document.querySelector("div.echo").scrollHeight) {
        document.querySelector(selector.body_css_real).style.paddingTop = (
            window.innerHeight - document.querySelector("div.echo")
            .scrollHeight) / 2 - 108 + "px";
    }
    if (window.innerWidth > (112 * echo_ell_length)) {
        body_align("navigation4limbo");
        document.querySelector(selector.body_css_real).style.paddingLeft = (
            window.innerWidth - (112 * echo_ell_length + 8 *
                echo_ell_overlay_length)) / 2 + "px";
        document.querySelector('div._footer').style.width = Math.floor((window
            .innerWidth / 112)) * 112 + "px";
        document.querySelector('div.ellCloseX').style.zIndex = "-1";
...
var parentNodeX = [
    ['Cloudflare', 'https://speed.cloudflare.com/', '_blank', '0',
    'better'],
    ...
    ['VENUSーＳ級熟女メーカ|Ｓ級熟女メーカー', 'https://venus-av.com/', '_blank', 'porn',
        'special'
    ],
    ['Glory Questー「禁断介護」や逆ショタ元祖', 'https://www.gloryquest.tv/', '_blank',
        'porn', 'special'
    ],
    ['Madonna（マドンナ）', 'https://www.madonna-av.com/top/', '_blank', 'porn',
        'common'
    ],
    ['SOD（ソフトオンデマンド）', 'https://www.sod.co.jp/', '_blank', 'poxrn',
        'common'],
    ['ATTACKERS（アタッカーズ）', 'https://www.attackers.net/top/', '_blank',
        'porn', 'common'
    ],
    ['PRESTIGE(プレステージ)', 'https://www.prestige-av.com/', '_blank', 'porn',
        'common'
    ],
    ['PREMIUM（プレミアム', 'https://www.premium-beauty.com/top/', '_blank',
        'porn', 'special'
    ],
    ['MOODYZー', 'https://www.moodyz.com/top/', '_blank', 'porn', 'common'],
    ['IDEAPOCKET (アイデアポケット）', 'https://www.ideapocket.com/top/', '_blank',
        'porn', 'porn', 'common'
    ],
    ['OPPAI（おっぱい）', 'https://www.oppai-av.com/', '_blank', 'porn',
    'common'],
    ['【kawaii*】公式サイト', 'https://www.kawaiikawaii.jp/top/', '_blank', 'porn',
        'common'
    ],
    ['肉感あふれる女優', 'https://www.fitch-av.com/top/', '_blank', 'porn',
        'common'],
    ['タカラ映像 TAKARA', 'https://www.takara-tv.jp/', '_blank', 'porn',
        'common'],
    ['ながえSTYLE(NTR)', 'https://www.nagae-style.com/', '_blank', 'porn',
        'common'
    ],
    ['トップページ - AVメーカー【ダスッ！】公式サイト', 'https://www.dasdas.jp/top/', '_blank',
        'porn', 'common'
    ],
    ['レズ・素人ナンパを中心', 'https://deeps.net/', '_blank', 'porn', 'common'],
    ['変態紳士倶楽部】公式サイト', 'https://www.to-satsu.com/top/', '_blank', 'porn',
        'common'
    ],
    ['wanzfactory（ワンズファクトリー）', 'https://www.wanz-factory.com/top/',
        '_blank', 'porn', 'common'
    ],
    ['【E-BODY（イーボディ）】公式サイト', 'https://www.av-e-body.com/top/', '_blank',
        'porn', 'common'
    ],
    ['MGS動画は', 'https://www.mgstage.com/', '_blank', 'porn', 'common'],
    ['ABC/妄想族(1302本)', 'https://www.mousouzoku-av.com/top/', '_blank',
        'porn', 'common'
    ],
    ['JET「卍GROUP」のトップページです', 'https://manji-group.com/top/', '_blank',
        'porn', 'common'
    ],
    ['!! ROCKET', 'https://www.rocket-inc.net/top.php', '_blank', 'porn',
        'common'
    ],
    ['FANZA通販-アダルト通販ショッピング', 'https://www.dmm.co.jp/mono/', '_blank',
        'porn', 'common'
    ],
    ['【無垢】公式サイトトーップページ | AVメーカ', 'https://www.muku.tv/top/', '_blank',
        'porn', 'common'
    ],
    ...
    ["Instagram", "https://www.instagram.com/", "_blank", "Social",
        'special'
    ],
    ["Tiktok", "https://www.tiktok.com/", "_blank", "Social", 'common'],
    ["Youtube", "https://m.youtube.com/", "_blank", "Social", 'special'],
    ["Netflix", "https://www.netflix.com/browse", "_blank", "Media",
        'special'
    ],
    ["HBO", "https://www.hbo.com/", "_blank", "Media", 'common'],
    ["Disney+", "https://www.disneyplus.com/en-hk", "_blank", "Media",
        'common'
    ],
    ["Amazon Prime Video", "https://www.primevideo.com/", "_blank", "Media",
        'common'
    ],
    ...
    ['MDN', 'https://developer.mozilla.org/zh-CN/', '_blank', 'Developer',
        'common'
    ],
    ['w3schools', 'https://w3schools.cn/', '_blank', 'Developer', 'common'],
    ['Stack Overflow', 'https://stackoverflow.com/', '_blank', 'Developer',
        'common'
    ],
    ['Typecho', 'https://typecho.org/', '_blank', 'front-build', 'special'],
    ['Vercel', 'https://vercel.com/new', '_blank', 'front-build',
    'special'],
    ['Cloudflare Pages', 'https://pages.cloudflare.com/', '_blank',
        'front-build', 'special'
    ],
    ['Gitpages', 'https://pages.github.com/', '_blank', 'front-build',
        'special'
    ],
    ['Fly.io', 'https://fly.io/', '_blank', 'front-build', 'common'],
    ['NameSilo', 'https://www.namesilo.com/', '_blank', 'domain-buy',
        'better'
    ],
    ['Cloudflare Domain', 'https://www.cloudflare.com/products/registrar/',
        '_blank', 'domain-buy', 'better'
    ],
    ['NameCheap', 'https://www.namecheap.com/', '_blank', 'domain-buy',
        'common'
    ],
    ['freenom', 'https://www.freenom.com/zh/freeandpaiddomains.html',
        '_blank', 'domain-buy', 'common'
    ],
    ...
    ['JavLibrary', 'https://www.javlibrary.com/cn/', '_blank', 'xOnline',
        'better'
    ],
    ...
    ['认知偏差手册',
        'https://s75w5y7vut.feishu.cn/docs/doccn3BatnScBJe7wD7K3S5poFf#RirzLG',
        '_blank', 'knowledge', 'common'
    ],
    ...
    ['ChatGPT', 'https://chat.openai.com/auth/login', '_blank', 'AICHAT',
        'better'
    ],
    ['Google Bard', 'https://bard.google.com/?hl=en', '_blank', 'AICHAT',
        'special'
    ],
    ['Claude.ai', 'https://claude.ai/', '_blank', 'AICHAT', 'special'],
    ['Google AI', 'https://labs.google.com/search/experiments', '_blank',
        'AICHAT', 'common'
    ],
    ['Bing AI',
        'https://www.bing.com/search?q=Bing+AI&showconv=1&FORM=hpcodx',
        '_blank', 'AICHAT', 'better'
    ],
    ["在线正则表达式测试", 'https://tool.oschina.net/regex/', '_blanl', 'Tools',
        'special'
    ],
    ['在线文件格式转换器', 'https://convertio.co/zh/', '_blank', 'Tools', 'common'],
    ['PDF在线转换', 'https://www.ilovepdf.com/zh-cn', '_blank', 'Tools',
        'common'
    ],
    ...
    ['博客优化', 'https://limbopro.com/category/builder/', '_blank',
        'seoandmore', 'special'
    ],
    ['博客防御', 'https://limbopro.com/tag/Cloudflare/', '_blank', 'seoandmore',
        'common'
    ],
    ['苦瓜书盘', 'https://kgbook.com/', '_blank', 'bookreadanddownload',
        'common'
    ],
    ['Library Genesis', 'https://www.libgen.is/', '_blank',
        'bookreadanddownload', 'special'
    ],
    ['JavaScript高级程序设计', 'https://t.me/limboprossr/2812', '_blank',
        'front-end', 'common'
    ],
    ['CSS教程', 'https://www.runoob.com/css/css-tutorial.html', '_blank',
        'front-end', 'common'
    ],
    ['w3school 在线教程', 'https://www.w3school.com.cn/index.html', '_blank',
        'front-end', 'common'
    ],
    ['M3U8下载?(Porn/Jable..)',
        'https://limbopro.com/archives/M3U8-Downloader.html', '_blank',
        'dload', 'common'
    ],
    ['Twitter 视频下载(PC网页版)', 'https://ssstwitter.com/', '_blank', 'dload',
        'common'
    ],
    ['Instagram 视频下载(电报🤖)', 'https://t.me/instasavegrambot', '_blank',
        'dload', 'special'
    ],
    ['YouTube 视频下载(电报🤖)', 'https://t.me/yt_dbot', '_blank', 'dload',
        'special'
    ],
    ['Instagram 视频下载(iOS捷径)', 'https://limbopro.com/archives/1053.html',
        '_blank', 'dload', 'special'
    ],
    ['Instagram 视频下载(PC网页版)', 'https://sssinstagram.com/', '_blank',
        'dload'],
    ['Youtube 视频下载(PC网页版)', 'https://ssyoutube.com/', '_blank', 'dload'],
    ['Pornhub 视频下载(PC网页版)', 'https://www.saveporn.net/', '_blank', 'dload'],
    ['More...', 'https://limbopro.com/category/downloader/', '_blank',
        'dload'
    ],
    ['Stable Diffusion入门',
        'https://limbopro.com/archives/install_and_quickstart_Stable_Diffusion.html',
        '_blank', 'aigc', 'special'
    ],
    ['Civitai', 'https://civitai.com/', '_blank', 'aigc', 'better'],
    ['Midjourney', 'https://midjourney.com/', '_blank', 'aigc', 'better'],
    ['Notion AI', 'https://www.notion.so/product/ai', '_blank', 'aigc',
        'better'
    ],
    ['网易新闻', 'https://news.163.com/', '_blank', 'currentnews', 'better'],
    ['谷歌新闻', 'https://news.google.com/home?hl=zh-CN&gl=CN&ceid=CN:zh-Hans',
        '_blank', 'currentnews', 'common'
    ],
    ['纽约时报', 'https://cn.nytimes.com/zh-hant/', '_blank', 'currentnews',
        'common'
    ],
    ['华尔街日报', 'https://cn.wsj.com/', '_blank', 'currentnews', 'common'],
    ['BBC News', 'https://www.bbc.com/zhongwen/simp', '_blank',
        'currentnews', 'common'
    ],
    ...
    ['pingwest', 'https://www.pingwest.com/', '_blank', 'technews',
        'common'],
];
var click_count = 0;
document.addEventListener("keydown", function(event) {
    if (event.code === "Escape") {
        ...
        if (!(document.querySelector('div#nsfw') === null) && !(document
                .querySelector('div#nsfw').style === null) && !(document
                .querySelector('div#nsfw').getAttribute('style') ===
                null) && (document.querySelector('div#nsfw')
                .getAttribute('style').search('-114') == -1) && document
            .querySelector('img.lockscreen') == null && click_count == 1
            ) {
            znsh_unlock();
        } else if ((document.querySelector(
                'div[data-chat-status="ongoing"]') && (document
                .querySelector('div[data-chat-status="ongoing"]')
                .getAttribute('data-visible') == 'true')) || document
            .querySelector('div[data-chat-status="initial"]') && (
                document.querySelector(
                    'div[data-chat-status="initial"]').getAttribute(
                    'data-visible') == 'true') && click_count == 1) {
            crisp_active('1');
        } else if (typeof body_build == 'function' && document
            .querySelector("#navigation4limbo").style.zIndex > 0 &&
            click_count == 1) {
            body_build('false');
        } else if (typeof close_googlesearch_iframe == 'function' &&
            document.querySelector("#searchbyGoogle") && (document
                .querySelector("#searchbyGoogle")).style.zIndex > 0 &&
            click_count == 1) {
            close_googlesearch_iframe();
        } else if ((document.querySelector("#x4Home").style.height ==
                "0%") && click_count == 1) {
            setTimeout(() => {
                x4Home_button('1')
            }, 1000)
            ...
            .querySelector("#navigation4limbo").style.zIndex < 0 &&
            click_count == 2 && (document.querySelector('div#nsfw') ==
                null || document.querySelector('div#nsfw').style
                .zIndex < 0)) {
            body_build('true')
        } else if (typeof body_build == 'function' && document
            .querySelector("#navigation4limbo").style.zIndex > 0 &&
    ...
    if (event.code === 'KeyG') {
        if (document.getElementById('navigation4limbo').style.zIndex >
            0 && (document.querySelector('.crisp-client.active') ===
                null)) {
            open_googlesearch_iframe();
        }
    }
    ...
    if (event.code === "Space") {
        if (!(document.querySelector('div#navigation4limbo').style
                .opacity == 0) && (document.querySelector(
                '.crisp-client.active') === null)) {
            open_googlesearch_iframe();
        }
    }
    if (event.code === 'Enter' && document.querySelector(
            'input.lockscreen') !== null) {
        screen_unlock();
    }
    setTimeout(() => {
        click_count = 0;
...
function new_align() {
    if (document.querySelectorAll('div.div_global').length > 15) {
        ...
        function sum(x) {
            let sum = 0;
            for (i = 0; i < x; i++) {
                sum += document.querySelectorAll('div.div_global')[i]
                    .clientWidth;
            }
            return sum;
        }
        ...
        if (sum(category) > window.innerWidth) {
            ...
            sum(category);
            document.querySelector(selector.body_css_real).style.paddingLeft = (
                window.innerWidth - sum(category)) / 2 + "px"
        } else {}
        document.querySelector(selector.body_css_real).style.paddingLeft = (
            window.innerWidth - sum(category)) / 2 + "px"
    }
    ...
    var parentElementX = setInterval(() => {
        if (document.querySelector('div#navigation4limbo[style]')) {
            ...
            if (nsfw_regex.test(window.location.href.toLowerCase())) {
                ...
                parent_push('.echo', 'news', '10', '时事新闻//',
                    'currentnews')
                parent_push('.echo', 'technews', '10', '科技新闻//',
                    'technews')
                parent_push('.echo', 'writer', '10', '广告与写作//',
                    'writer')
                parent_push('.echo', 'bookreadanddownload', '4',
                    '电子书//', 'bookreadanddownload')
                parent_push('.echo', 'seoandmore', 4, "建站指北//",
                    "seoandmore")
                parent_push('.echo', 'front-build', 4, "建站工具//",
                    "front-build")
                parent_push('.echo', 'domain-buy', 4, "域名注册商//",
                    "domain-buy")
                parent_push(".echo", 'DeverloperX', 4, '开发者社区//',
                    'Developer')
                parent_push(".echo", 'Images', 4, '免费商用图片', 'IMages')
                child_push('.div_global.front > ul', '', "",
                    'front-end', 0)
                child_push('.div_global.boysshouldread > ul', '', '',
                    'knowledge', 0)
                child_push('.div_global.ipcheck > ul', '', '',
                    'ipcheck', 1)
                scroll_switch();
                _footer_move();
                csp_remove();
                ...
                parent_push(".echo", 'Images', 4, '免费商用图片', 'IMages')
                parent_push('.echo', 'PornMaker', 4, '著名片商//', 'porn')
                parent_push('.echo', 'xOnline', 4, 'Adult//', 'xOnline')
                parent_push('.echo', 'comic18', 4, '漫画//', 'comic18')
                child_push('.div_global.front > ul', '', "",
                    'front-end', 0)
                child_push('.div_global.boysshouldread > ul', '', '',
...
function nsfw_content_hidden(x) {
    if (x == 'true') {
        if (!document.querySelector("#nsfw_echo")) {
            ...
            let parentElement = document.querySelector('#x4Div') ? document
                .querySelector('#x4Div') : '';
            parentElement.appendChild(nsfw_frame_blur)
            if (window.innerWidth < window.innerHeight) {
                var innerHTML_echo =
                    '<div id="nsfw_echo"><img class="nsfw" src="https://limbopro.com/Ad_swipe_mobile_1.png"></div>'
            } else {
                var innerHTML_echo =
                    '<div id="nsfw_echo"><img class="nsfw" src="https://limbopro.com/Ad_swipe_pc_1.png"></div>'
            }
...
function visibility() {
    if (getCookie('nsfwmode') !== 'false') {
        document.addEventListener('visibilitychange', function() {
            var visible = document.visibilityState;
            if (visible !== 'visible' && getCookie('nsfwmode') !==
                'false') {
                document.body.style.filter = 'blur(50px)';
...
function sessionX(x) {
    if (x == 'true') {
        if (document.getElementById('nsfw') && document.getElementById(
            'nsfw') !== null && document.getElementById('nsfw').style.zIndex > 0
            ) {
...
function visibility_switch() {
    var znsh_setInterval = window.setInterval(() => {
        if (document.querySelector("div#nsfw")) {
            document.querySelector("div#nsfw").addEventListener("click",
                znsh_unlock);
        ...
        if (document.querySelector("#nsfw")) {
            if (document.body.style.filter.replace(/[^0-9]/ig, "") > 10) {
                var filter_now = document.body.style.filter.replace(/[^0-9]/ig,
                    "");
                document.body.style.filter = 'blur(' + (filter_now - 20) +
                'px)';
            } else {
...
function nsfwmode_check() {
    if (getCookie('nsfwmode') !== 'false' && nsfw_regex.test(document.location
            .href)) {
        znsh();
        if (document.getElementById('nsfwmode_switch')) {
            znsh();
            document.getElementById('nsfwmode_switch').textContent =
                '成人保护模式(ON)';
            setTimeout(() => {
                body_build('false');
            }, 1200)
        }
    } else if (getCookie('nsfwmode') !== 'true' && nsfw_regex.test(document
            .location.href)) {
        if (document.getElementById('nsfwmode_switch')) {
            znsh_unlock();
            document.getElementById('nsfwmode_switch').textContent =
...
function hide_button_switch(x) {
    if (x == 1) {
        ...
        var days = window.prompt(
            "请输入你想要隐藏该按钮的天数？（请直接输入数字），该按钮将在你指定的时间后再次显示。你可通过清理浏览器 cookie 或 无痕模式打开的方式提前停止计时。本次设置仅针对当前网站域名生效。"
            );
...
function lock_screen_setPWD(x) {
    if (x == 'check') {
        lock_screen_mode_check();
    } else if (getCookie('lock_screen_pwd') == '' || getCookie(
            'lock_screen_pwd') == 'null') {
        var password = window.prompt(
            "请设置锁屏密码（任意你记得住的字母/数字等符号的简单组合)；如忘记密码，你可通过清理浏览器 cookie 的方式重置密码；本次设置仅针对当前网站域名生效。"
            );
...
function screen_unlock() {
    if (document.querySelector('input.lockscreen').value == getCookie(
            'lock_screen_pwd')) {
        ...
        nsfwmode('true');
        document.querySelector("button.unlock").removeEventListener("click",
            screen_unlock);
        document.querySelector('img.lockscreen').className = 'nsfw';
        document.querySelector('img.nsfw').style = 'filter:blur(0px)';
        setTimeout(() => {
            document.querySelector("img.nsfw").addEventListener("click",
                znsh_unlock());
        }, 0)
...
function lock_screen_mode_check_after() {
    if (getCookie('lock_screen_mode') == 'true') {
        ...
        button_echo.style =
            'border-radius:4px; font-size:medium; border-radius:26px; box-shadow:inset 0px 0px 15px 3px #16191f00; position: relative;z-index: 114155;top: 50%;width: 180px;height: 40px; font-weight:inherit; background:blue; color:white;';
        button_echo.textContent = 'UNLOCK!';
        var input_echo = document.createElement('input');
        input_echo.className = 'lockscreen';
        input_echo.style =
            '-webkit-text-security:disc;z-index:114155; position:fixed;width:180px;height:27px;top:45%;outline-style:none;border:0px;color:black;background:aliceblue;auto-complete:new-password;';
        ...
        document.querySelector("button.unlock").addEventListener("click",
            screen_unlock);
        body_build('false');
    }
}
...
function parentElement_build(parentNode, i) {
    if (document.querySelectorAll(parentNode)[0]) {
        let parentElement = document.querySelectorAll(parentNode)[0];
        let node = document.querySelectorAll(parentNode)[0].children[i];
        let clone = node.cloneNode(true);
        parentElement.appendChild(clone);
    }
}

function parentElement_inner(parentNode, Headline_css, Headreplace_str,
    child_css_change) {
    document.querySelector(".div_global." + child_css_change).querySelector(
...
function parentElement_push(parentNode, category, child_css_change,
    ...
    var real_length = document.querySelector(".div_global." + child_css_change +
        " > ul").querySelectorAll('li').length
    if (number > real_length) {
        var lenth_now = number - real_length;
        for (i = 0; i < lenth_now; i++) {
            li_sample(".div_global." + child_css_change + " > ul",
                insertBefore_that_element, switchX);
        }
        var bb = 0;
        for (i = 0; i < arrayXP.length; i++) {
            if (arrayXP[i][3] == category) {
                document.querySelector(".div_global." + child_css_change)
                    .querySelectorAll('a')[bb].textContent = arrayXP[i][0];
                document.querySelector(".div_global." + child_css_change)
                    .querySelectorAll('a')[bb].href = arrayXP[i][1];
                document.querySelector(".div_global." + child_css_change)
                    .querySelectorAll('a')[bb].target = arrayXP[i][2];
                document.querySelector(".div_global." + child_css_change)
                    .querySelectorAll('a')[bb].id = arrayXP[i][3];
                document.querySelector(".div_global." + child_css_change)
                    .querySelectorAll('a')[bb].classList.add(arrayXP[i][3]);
                document.querySelector(".div_global." + child_css_change)
                    .querySelectorAll('a')[bb].classList.add(arrayXP[i][4]);
                var bb = ++bb;
            }
        }
    } else {
...
function parent_push(parentNode, child_css_change, childPosition, title,
    category, insertBefore_that_element, switchX) {
    div_sample(parentNode, insertBefore_that_element, switchX,
    child_css_change);
    parentElement_inner(parentNode, '.title_global', title, child_css_change);
    parentElement_push(parentNode, category, child_css_change,
        insertBefore_that_element, switchX)
}

function child_build(div_global_selector) {
    if (document.querySelector(div_global_selector)) {
        let parentElement = document.querySelector(div_global_selector);
        let node = document.querySelector(div_global_selector).lastElementChild;
        let clone = node.cloneNode(true);
        parentElement.appendChild(clone);
    }
}
...
function child_inner(div_global_selector, length, array, category,
    insertBefore_that_element, switchX) {
    if (parentNodeX[length][3] == category) {
        li_sample(div_global_selector, insertBefore_that_element, switchX);
        document.querySelector(div_global_selector).lastElementChild
            .querySelector('a').textContent = parentNodeX[length][0]
        document.querySelector(div_global_selector).lastElementChild
            .querySelector('a').href = parentNodeX[length][1]
        document.querySelector(div_global_selector).lastElementChild
            .querySelector('a').target = parentNodeX[length][2]
    }
}

function child_push(div_global_selector, length, array, category,
    insertBefore_that_element, switchX) {
    for (i = 0; i < parentNodeX.length; i++) {
        child_inner(div_global_selector, i, array, category,
            insertBefore_that_element, switchX);
    }
}
...
function echo_check_switch(x) {
    if (x == 1) {
        var echo_check = setInterval(() => {
            ...
            setTimeout(() => {
                if (document.querySelector(
                        "#navigation4limbo[style]")) {
                    if (document.querySelector(
                            "#navigation4limbo[style]").style
                        .opacity == 1) {
        ...
        clearInterval(echo_check)
    }
}
const js_common_fx = {
    crisp: 'https://limbopro.com/Adguard/crisp.js'
}
...
function crisp_window_switch() {
    ...
    let crisp_visible = setInterval(() => {
        if (document.querySelectorAll("[data-visible='true']").length >
            1) {
            crisp_window_remove("1");
            clearInterval(crisp_visible);
        } else {
            if (document.querySelectorAll("[data-visible='true']")
                .length == 1) {
                crisp_window_remove("0");
                clearInterval(crisp_visible);
            }
...
function crisp_window_remove(x) {
    if (x == 0) {
        if (document.querySelector("[data-for-id=new_messages]") !== null &&
            document.querySelector('[aria-live=polite].crisp-client')
            .classList !== null) {
            document.querySelector('[aria-live=polite].crisp-client').classList
                .add('active');
            document.querySelector('[aria-live=polite].crisp-client').style =
                'z-index:115155!important';
            ...
            if (document.querySelector('[aria-live=polite].crisp-client') !==
                null && document.querySelector(
                    '[aria-live=polite].crisp-client').classList !== null) {
                document.querySelector('[aria-live=polite].crisp-client')
                    .classList.remove('active');
                document.querySelector('[aria-live=polite].crisp-client')
                    .style = 'z-index:-115155!important';
            }
        }
    } else if (x == 1) {
        if (document.querySelector('[aria-live=polite].crisp-client') !==
            null && document.querySelector('[aria-live=polite].crisp-client')
            .classList !== null) {
            document.querySelector('[aria-live=polite].crisp-client').classList
                .add('active');
            document.querySelector('[aria-live=polite].crisp-client').style =
    ...
    if (x == 1) {
        let crisp_check_2 = setInterval(() => {
            if (!document.querySelector("script[src*='crisp']")) {
                thrd_party_file("script",
                    "https://limbopro.com/Adguard/crisp.js", "head");
                crisp_window_remove('1');
                crisp_window_switch();
...
function crisp_auto_hidden() {
    val = null
    if (!document.querySelector("[data-for-id=new_messages]") && val == document
        .querySelector('.cc-1hqb[data-visible=true]')) {
        crisp_window_remove('0');
    } else {
        if (document.querySelector("[data-for-id=new_messages]")) {
            crisp_window_remove('0');
...
function testx() {
    var selector = window.prompt(
        "请输入你想要移除的元素对应的标签 e.g. div a li ul 或更具体的元素选择器 e.g. .ad #ad ");
...
function close_googlesearch_iframe_eventlistener() {
    let mikey = setInterval(() => {
        if (document.querySelector('button#close_search_button') !==
            null) {
            document.querySelector('button#close_search_button')
                .addEventListener('click', function() {
                    close_googlesearch_iframe();
                });
            clearInterval(mikey);
...
function open_googlesearch_iframe() {
    ...
    if (document.cookie.indexOf("alert") == -1) {
        setCookie('alert', '7', '7')
        alert(
            '功能呈现页尚处于优化中...，欢迎反馈使用体验及建议...(搜索结果基于谷歌，如需更好的体验可多刷新/重新加载几次搜索页面以便谷歌自动化完善样式...)')
    }
    if (document.querySelector('#searchbyGoogle')) {
        ...
        document.querySelector('#x4Home').style.bottom = '30%';
        setTimeout(() => {
            if (!document.querySelectorAll("script[src*='=a897efc85e']")
                .length >= 1) {
                let parentElement = document.querySelector(
                    '#searchbyGoogle');
                let new_s = document.createElement('script');
                new_s.src =
                    'https://cse.google.com/cse.js?cx=a897efc85ee66bfdd';
        ...
        if (!csp_regex.test(window.location.href.toLowerCase())) {
            ...
            document.querySelector('#searchbyGoogle').appendChild(
                close_search_button);
            close_googlesearch_iframe_eventlistener();
            googlesearch_blank();
        } else {
...
function googlesearch_blank() {
    var googlesearch_blank_check = setInterval(() => {
        console.log("定时器正在执行...")
        if (document.querySelector("body#searchbyGoogle").style
            .zIndex == -11415411) {
...
function close_googlesearch_iframe() {
    if (document.querySelectorAll(
        ...
        0]) {
        if (document.querySelectorAll("div[class*='gsc-results-close-btn']")[
            0]) {
            document.querySelectorAll("div[class*='gsc-results-close-btn']")[0]
                .click();
        }
    } else if (!(/\b(gsc.q)\b/i.test(document.location.href))) {
        console.log('谷歌搜索已关闭...')
        ...
        if (document.querySelector('#searchbyGoogle')) {
            ...
            document.querySelector('#x4Home').style.bottom = '15%';
        }
    } else {
        open_googlesearch_iframe()
    }
...
function copyText(id1, id2, Text) {
    ...
    setTimeout(() => {
        ...
        ele_2.style.backgroundColor = corlor.css
        .backgroundColor_recover;
        border_color(ele_array, corlor.css.borderRight_recover)
    }, 3000);
}

function ele_dynamicAppend(selector, attribute, txt, style, func, id, array,
    tag) {
